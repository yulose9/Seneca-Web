...
const StreakCalendar = ({ habit, color, history = {}, onToggle }) => {
    ...
    const stats = useMemo(() => {
        const sortedDates = Object.keys(history).filter(d => history[d]).sort().reverse();
        let currentStreak = 0;
            ...
            <div className="flex items-center justify-between mb-5 px-1">
                ...
                <div className="flex gap-6">
                    <div className="text-right">
                        <p className="text-[11px] font-bold text-[rgba(60,60,67,0.4)] uppercase tracking-wide">Streak</p>
                        <p className="text-[20px] font-bold leading-none tabular-nums" style={{ color }}>{stats.currentStreak}</p>
                    </div>
                    <div className="text-right">
                        <p className="text-[11px] font-bold text-[rgba(60,60,67,0.4)] uppercase tracking-wide">Total</p>
                        <p className="text-[20px] font-bold leading-none tabular-nums" style={{ color }}>{stats.total}</p>
                    </div>
                </div>
            </div>
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            style={{
                                                width: BOX_SIZE,
                                                height: BOX_SIZE,
                                                backgroundColor: isDone ? color : 'rgba(120,120,128,0.06)',
                                                border: isToday && !isDone ? `2px solid ${color}` : 'none',
                                        ...
                                        >
                                            ...
                                            <span
                                                ...
                                                style={{ color: isToday && !isDone ? color : undefined }}
                                            >
                                                {dayNum}
                                            </span>
            ...
            <div className="flex items-center justify-between mt-4 px-2">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[12px] text-[rgba(60,60,67,0.5)] font-medium">Missed</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: color, boxShadow: `0 2px 8px ${color}40` }} />
                        <span className="text-[12px] text-[rgba(60,60,67,0.5)] font-medium">Done</span>
                    </div>
                </div>
            </div>
        </div>
...
export default function HabitDetailSheet({ visible, onClose, habit, onToggleHistory, getHistory }) {
                        ...
                        <div className="ios-sheet-content max-h-[85vh] overflow-y-auto px-6 pb-12 pt-2">
                                        ...
                                        {habitInfo.certifications.map((cert, i) => (
                                            <div
                                                ...
                                                className={clsx(
                                                    ...
                                                    i !== habitInfo.certifications.length - 1 && "border-b border-[rgba(60,60,67,0.08)]"
                                                )}
                                            >
                                                <div className={clsx(
                                                    "w-10 h-10 rounded-full flex items-center justify-center mr-4",
                                                    cert.status === 'done' ? "bg-[#D1FAE5]" :
                                                        cert.status === 'progress' ? "bg-[#FEF3C7]" : "bg-[#F3F4F6]"
                                                )}>
                            ...
                            {habitInfo.action === 'journal' && (
                                <motion.button
                                    ...
                                    style={{ borderColor: habitInfo.color, color: habitInfo.color }}
                                >
                                    Go to Journal 📖
                                </motion.button>
                            )}
                            ...
                            <div className="mt-8 pb-8">
                                {!isTodayDone && habitInfo.type === 'choice' ? (
                                    ...
                                    <motion.button
                                        ...
                                        style={{
                                            backgroundColor: isTodayDone ? habitInfo.color : undefined,
                                            boxShadow: isTodayDone ? `0 6px 20px ${habitInfo.color}50` : undefined
                                        }}
                                    >
                                        {isTodayDone ? (
                                            <>
