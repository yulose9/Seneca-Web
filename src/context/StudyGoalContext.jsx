...
export function StudyGoalProvider({ children }) {
    // Active study goal (selected certificate)
    const [activeStudyGoal, setActiveStudyGoal] = useState(null);

    // Study history: { [dateStr]: true/false/undefined }
    ...
    const getStudyStreak = useCallback(() => {
        ...
        for (let i = 0; i < 365; i++) {
            ...
            if (studyHistory[dateStr] === true) {
                streak++;
            } else if (studyHistory[dateStr] === false || (i > 0 && studyHistory[dateStr] === undefined)) {
